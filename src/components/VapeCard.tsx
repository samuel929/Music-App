...
const VapeProductCard = ({ name, id, imageUrl }: VapeProduct) => {
  return (
    <Link href={`/merch/${id}`}>
      <div className='relative w-full sm:w-80 h-[450px] sm:h-[450px] bg-gray-900 rounded-lg overflow-hidden m-4'>
        ...
        <div className='absolute bottom-0 w-full h-24 sm:h-24 text-center bg-white/50 backdrop-blur-md transition-all duration-1000 z-20 hover:h-52 sm:hover:h-52'>
          ...
          <div className='inline-block mt-2 sm:mt-2 px-4 py-2 bg-white text-black font-semibold rounded-md opacity-0 transform translate-y-12 transition-all duration-700 hover:opacity-100 hover:translate-y-0'>
            Buy Now
          </div>
        </div>
      </div>
